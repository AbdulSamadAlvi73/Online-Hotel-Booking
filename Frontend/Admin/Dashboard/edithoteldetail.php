...
include("./conn.php");
$id=$_GET['id'];
$sel="SELECT * FROM `hotel_details` WHERE id='$id'";
...
if(isset($_POST['submit'])){
 ...
 move_uploaded_file($image_temp, $target);
  ...
  $query = "UPDATE `hotel_details` SET `logo`='$target', `manager_name`='$manager', `manager_img`='$target0', `img1`='$target1', `img2`='$target2', `img3`='$target3', `img4`='$target4', `img5`='$target5', `img6`='$target6' WHERE `id`='$id'";
  $query_run = mysqli_query($conn, $query);

  if($query_run) {
      header('location:viewhoteldetails.php');
  } else {
 ...
 <!-- End Sidebar-->
  ...
  <main id="main" class="main">
    ...
    <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">Edit Hotal Details</h5>

            <!-- Vertical Form -->
            <form class="row g-3" action="#" method="POST" enctype="multipart/form-data">
            <div class="col-12">
                <label for="logo" class="form-label">Hotel Logo</label>
                <input type="file" class="form-control" id="logo" name="logo"  value="<?php echo $row['logo']?>">
              </div>
              <div class="col-12">
                <label for="manager" class="form-label">Manager Name</label>
                <input type="text" class="form-control" id="manager" name="manager"  value="<?php echo $row['manager_name']?>">
              </div>
              <div class="col-12">
                <label for="manager_img" class="form-label">Manager Picture</label>
                <input type="file" class="form-control" id="manager_img" name="manager_img"  value="<?php echo $row['manager_img']?>">
              </div>
              <!-- <div class="col-12">
              ...
              <div class="col-6">
                ...
                <input type="file" class="form-control" id="image1" name="image1" value="<?php echo $row['img1']?>">
              </div>
              <div class="col-6">
                <label for="image2" class="form-label">Slider Second Image</label>
                <input type="file" class="form-control" id="image2" name="image2" value="<?php echo $row['img2']?>">
              </div>  
              <div class="col-6">
                <label for="image3" class="form-label">Slider Third Image</label>
                <input type="file" class="form-control" id="image3" name="image3" value="<?php echo $row['img3']?>">
              </div>  
              <div class="col-6">
                <label for="image4" class="form-label">Slider Fourth Image</label>
                <input type="file" class="form-control" id="image4" name="image4" value="<?php echo $row['img4']?>">
              </div>  
              <div class="col-6">
                <label for="image5" class="form-label">Slider Fiveth Image</label>
                <input type="file" class="form-control" id="image5" name="image5" value="<?php echo $row['img5']?>">
              </div> 
               <div class="col-6">
                <label for="image6" class="form-label">Slider Sixth Image</label>
                <input type="file" class="form-control" id="image6" name="image6" value="<?php echo $row['img6']?>">
              </div>
